...
void SystemStub_SDL::init(const char *title, int w, int h) {
	...
	_screenH = h;
#ifdef OPENDINGUX
	_offscreenSize = 640 * 480 * sizeof(uint32);
#else
...
void SystemStub_SDL::updateScreen() {
	...
	if (_blurOn) {
		...
		for (int i = 0; i < _blitRectsCount; ++i) {
			SDL_Rect *br = &_blitRects[i];
#ifdef OPENDINGUX
			uint32 *dst = (uint32*)_screen->pixels + (br->y>>1)*(_screenW>>1) + (br->x>>1);
			uint32 *src = _offscreen + br->y * _screenW + br->x;
			for (int h = 0; h < (br->h>>1); h++){
				for (int w = 0; w < (br->w>>1); w++){
					dst[h*(_screenW>>1) + w] = src[(h<<1)*_screenW + (w<<1)];
				}
			}
#else
	...
	//SDL_Flip(_screen);
  {
    SDL_Surface* p = SDL_ConvertSurface(_screen, ScreenSurface->format, 0);
...
void SystemStub_SDL::unlockYUV() {
	if (_yuv && _yuvLocked) {
		...
		SDL_Rect r;
#ifdef OPENDINGUX
		if (_yuv->w * 2 <= (_screenW>>1) && _yuv->h * 2 <= (_screenH>>1)) {
#else
...
#ifdef OPENDINGUX
		r.x = ((_screenW>>1) - r.w) / 2;
		r.y = ((_screenH>>1) - r.h) / 2;
#else
		r.x = (_screenW - r.w) / 2;
		r.y = (_screenH - r.h) / 2;
#endif
		...
		while (SDL_PollEvent(&ev)) {
			...
			case SDL_KEYDOWN:
				...
				case SDLK_DOWN:
					...
					break;
#ifdef OPENDINGUX
				case SDLK_RETURN: //START => open bag = tab
					_pi.tab = true;
					break;
				case SDLK_SPACE: //X => show stats = ctrl
					_pi.ctrl = true;
					break;
				case SDLK_LSHIFT: //Y => run and hide weapon = shift
					_pi.shift = true;
					break;
				case SDLK_LCTRL: //A => use weapon = space
					_pi.space = true;
					break;
				case SDLK_LALT: //B => action and skip video/dialog = enter
					_pi.enter = true;
					break;
				case SDLK_ESCAPE: //SELECT => quit
					_quit = true;
					break;
				case SDLK_BACKSPACE: //L => load
					_pi.load = true;
					break;
				case SDLK_TAB: //R => save
					_pi.save = true;
					break;
#else
				case SDLK_RETURN:
...
void SystemStub_SDL::setScreenDisplay(bool fullscreen) {
  ...
  _screen = SDL_CreateRGBSurface(SDL_SWSURFACE, _screenW>>1, _screenH>>1, kVideoSurfaceDepth, 0, 0, 0, 0);
#else
